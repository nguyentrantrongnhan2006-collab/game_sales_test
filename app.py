...
Yêu cầu file "vgsales_model_artifact.joblib" nằm cùng thư mục.
"""
...
# ----------------------------------------------------------------------
# 1. LOAD ARTIFACT (model + các map encoding) — cache để không load lại
# ----------------------------------------------------------------------
@st.cache_resource
def load_artifact(path="vgsales_model_artifact.joblib"):
...
try:
    ...
    load_error = (
        "Không tìm thấy file `vgsales_model_artifact.joblib`. "
        "Hãy đặt file này **cùng thư mục** với app.py rồi chạy lại "
        "(kiểm tra bằng lệnh `dir` / `ls` xem file có thật sự nằm đó không)."
    )
except Exception as e:  # bắt luôn các lỗi khác khi load (ví dụ file bị hỏng, thiếu quyền đọc...)
    load_error = f"Lỗi khi tải file model: {e}"
...
if artifact is None:
    # Phòng hờ trường hợp st.stop() ở trên không dừng được script (một số môi trường/cấu hình đặc biệt)
    raise RuntimeError(
        "Không thể tải artifact model và st.stop() không dừng được script. "
        "Hãy kiểm tra lại file vgsales_model_artifact.joblib và cách bạn khởi chạy ứng dụng "
        "(chạy đúng lệnh `streamlit run app.py`, không thêm tham số thừa phía sau)."
    )
...
tab_predict, tab_eda, tab_performance, tab_about = st.tabs(
    ["🔮 Dự đoán", "📁 Dữ liệu & Khai phá", "📊 Hiệu suất mô hình", "ℹ️ Giới thiệu"]
)
